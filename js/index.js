  ...
  if (selectedStream != currentStream) {
    ...
    switch (streams.stream[currentStream].id) {
      ...
      case 'wikipedia':
        currentListener = {
          message: payload => {
            addFormattedMessageWikipedia(payload)
          }
        }
      ...
      case 'gamestate':
        currentListener = {
          message: payload => {
            addFormattedMessageGameState(payload)
          }
        }
        pubnub.addListener(currentListener)
        break
      case 'sensornetwork':
        currentListener = {
          message: payload => {
            addFormattedMessageSensorNetwork(payload)
          }
        }
        pubnub.addListener(currentListener)
        break
      case 'marketorders':
        currentListener = {
          message: payload => {
            addFormattedMessageMarketOrders(payload)
          }
        }
        pubnub.addListener(currentListener)
        break
...
function addFormattedMessageWikipedia (payload) {
  ...
  var outputHtml = ''
  outputHtml +=
    "<b>Source: </b>Wikipedia <i class='fa-brands fa-wikipedia-w fa-2xl' style='float:right'></i><br>"
...
function addFormattedMessageGameState (payload) {
  ...
  var outputHtml = ''
  outputHtml +=
    "<b>Source: </b>Game State <i class='fa-solid fa-gamepad fa-2xl' style='float:right'></i><br>"
...
function addFormattedMessageSensorNetwork (payload) {
  ...
  var outputHtml = ''
  outputHtml +=
    "<b>Source: </b>Sensor Network <i class='fa-solid fa-temperature-half fa-2xl' style='float:right'></i><br>"
  outputHtml +=
...
function addFormattedMessageMarketOrders (payload) {
  ...
  var outputHtml = ''
  outputHtml +=
    "<b>Source: </b>Market Orders <i class='fa-solid fa-money-bill-trend-up fa-2xl' style='float:right'></i><br>"
  ...
  outputHtml +=
    ...
    '<br>'
  outputHtml +=
    '<b>Order Quantity: </b>' + payload.message.order_quantity + '<br>'
